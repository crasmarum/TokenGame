...
\subsection{Step B: seating by slides (polynomial when \texorpdfstring{$R,S$}{R,S} known)}\label{sec:stepB}
...
\section{A worked example: factoring \texorpdfstring{$143$}{143}}\label{sec:example}
...
metrics below), the Java reference solver, and the paper source are available at
\url{https://github.com/crasmarum/TokenGame}.} It does not, and \emph{why} it does not mirrors
...
\section{Binary operation on weights, and the full-or-\texorpdfstring{$R$}{R} variant}\label{sec:hash}
